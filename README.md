...
| **Portal Launcher** | A start button that opens a full-screen launcher: the desktop dims and a centered card animates in. **Home** (pinned apps, recently used, continue playing with Steam art and friend glow, recent files), **Apps** (every category), **Games** (your library with art, played/friends filters), **Files** (places, recent documents and folders), **Friends** (in game / online / offline) and **System** (lock, sleep, restart, shut down with a second press to confirm, settings shortcuts). One search finds apps, games, friends, settings pages, files, calculations and commands; prefixes narrow it (`g ` games, `f ` files, `a ` apps, `@` friends, `=` math, `>` command). Full keyboard control, right-click actions (pin, app jump lists, add to panel/desktop, edit, uninstall). Meta opens it when it replaces the application launcher. Uses no CPU while closed. |
