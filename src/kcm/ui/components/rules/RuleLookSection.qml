...
ColumnLayout {
    ...
    readonly property var opacityNode: kcm.revision >= 0 ? kcm.node(rulePath + "/opacity") : ({})
    readonly property var radiusNode: kcm.revision >= 0 ? kcm.node(rulePath + "/geometry-corner-radius") : ({})
    ...
    SettingRow {
        ...
        RowLayout {
            Segmented {
                ...
                onChosen: value => value === "default" ? kcm.remove(section.rulePath + "/opacity") : kcm.setValue(section.rulePath + "/opacity", [0.9])
            }
            ...
            QQC2.Slider {
                ...
                onPressedChanged: {
                    if (!pressed) {
                        kcm.setValue(section.rulePath + "/opacity", [Number(value.toFixed(2))]);
                    }
                }
            }
    ...
    SettingRow {
        ...
        RuleCornersField {
            ...
            onEdited: values => values === null
                ? kcm.remove(section.rulePath + "/geometry-corner-radius")
                : kcm.setValue(section.rulePath + "/geometry-corner-radius", values.every(value => value === values[0]) ? [values[0]] : values)
        }
    }
