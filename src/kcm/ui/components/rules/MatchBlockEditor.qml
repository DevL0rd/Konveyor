...
ColumnLayout {
    ...
    readonly property var profileNames: kcm.revision >= 0 ? kcm.children("", "monitor-profile").map(entry => String((entry.node.args || [])[0] || "")).filter(name => name.length) : []
    ...
    function write(changes) {
        ...
        kcm.setNode(path, { name: node.name || "match", args: [], props: next });
    }
