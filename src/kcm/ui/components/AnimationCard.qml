...
Card {
    ...
    readonly property var params: kcm.values[base] || ({ enabled: true, kind: "easing", "duration-ms": 250, curve: "ease-out-cubic", bezier: [0, 0, 1, 1] })
    readonly property bool isSpring: params.kind === "spring"
    readonly property bool masterOn: kcm.values["animations/enabled"] === true
    readonly property real slowdown: kcm.values["animations/slowdown"] || 1
    ...
    function writeSpring(changes) {
        ...
        kcm.remove(base + "/duration-ms");
        kcm.remove(base + "/curve");
        kcm.setValue(base + "/spring", [], {
    ...
    function writeCurve(curve, bezier) {
        kcm.remove(base + "/spring");
        if (!kcm.has(base + "/duration-ms")) {
            kcm.setValue(base + "/duration-ms", [Math.round(isSpring ? 250 : params["duration-ms"])]);
        }
        kcm.setValue(base + "/curve", curve === "cubic-bezier" ? ["cubic-bezier"].concat(bezier) : [curve]);
    }
    ...
    SettingRow {
        ...
        QQC2.Switch {
            ...
            onToggled: kcm.setFlag(card.base + "/off", !checked)
    ...
    SliderRow {
        ...
        onEdited: value => kcm.setValue(card.base + "/duration-ms", [Math.round(value)])
    }
