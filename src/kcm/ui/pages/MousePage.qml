...
SettingsPage {
    ...
    readonly property var corners: kcm.values["gestures/hot-corners"] || ({})
    readonly property var viewScroll: kcm.values["gestures/dnd-edge-view-scroll"] || ({})
    readonly property var workspaceSwitch: kcm.values["gestures/dnd-edge-workspace-switch"] || ({})
    readonly property string warpChoice: kcm.values["input/warp-mouse-to-focus"] !== true ? "off" : (kcm.values["input/warp-mouse-to-focus/mode"] || "nearest")
    ...
    function toggleCorner(corner) {
        ...
        if (!cornerIds.some(id => next[id])) {
            kcm.setFlag("gestures/hot-corners/off", true);
            return;
        }
        for (const id of cornerIds) {
            kcm.setFlag("gestures/hot-corners/" + id, next[id]);
        }
    }

    function writeWarp(choice) {
        if (choice === "off") {
            kcm.remove("input/warp-mouse-to-focus");
        } else {
            kcm.setValue("input/warp-mouse-to-focus", [], choice === "nearest" ? {} : { mode: choice });
        }
    }
    ...
    Card {
        SwitchRow {
            ...
            isOn: kcm.values["input/focus-follows-mouse"] === true
            onSwitched: on => kcm.setFlag("input/focus-follows-mouse", on)
        }
    ...
    Card {
        SettingRow {
            ...
            ChoiceCards {
                ...
                currentValue: kcm.values["gestures/titlebar-drag"]
                options: [
    ...
    Card {
        SwitchRow {
            ...
            onSwitched: on => kcm.setFlag("gestures/hot-corners/off", !on)
        }
