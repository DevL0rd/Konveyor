...
SettingsPage {
    ...
    title: "Monitors"
    readonly property var profiles: kcm.revision >= 0 ? kcm.children("", "monitor-profile") : []
    readonly property var overrides: kcm.revision >= 0 ? kcm.children("", "output") : []
    ...
    function usedBy(name) {
        return kcm.live.outputs.filter(output => kcm.profileForOutput(output) === name).map(output => output.name);
    }
    ...
    NamePrompt {
        ...
        onNamed: name => {
            const path = kcm.append("", Kdl.block("monitor-profile", [], [name]));
            if (path.length) {
                kcm.push("pages/MonitorProfilePage.qml", { profilePath: path });
            }
        }
    }
    ...
    Card {
        ...
        Repeater {
            ...
            ListCard {
                ...
                onOpened: kcm.push("pages/MonitorProfilePage.qml", { profilePath: modelData.path })
                onMovedUp: kcm.move(modelData.path, -1)
                onMovedDown: kcm.move(modelData.path, 1)
                onDeleted: kcm.remove(modelData.path)
            }
        }
    }
    ...
    Card {
        ...
        Repeater {
            ...
            ListCard {
                ...
                heading: MonitorSummary.nameOf(modelData)
                subtitle: {
                    const connected = kcm.live.outputs.find(output => output.name === heading);
                ...
                active: kcm.live.outputs.some(output => output.name === heading)
                showOrder: false
                onOpened: kcm.push("pages/OutputOverridePage.qml", { outputPath: modelData.path })
                onDeleted: kcm.remove(modelData.path)
            }
        }
        ...
        RowLayout {
            ...
            OutputPicker {
                id: newOutput
                value: kcm.live.outputs.map(output => output.name).find(name => !page.overrideNames.includes(name)) || ""
            ...
            QQC2.Button {
                ...
                onClicked: {
                    const path = kcm.append("", Kdl.block("output", [], [newOutput.value]));
                    if (path.length) {
                        kcm.push("pages/OutputOverridePage.qml", { outputPath: path });
                    }
                }
            }
