...
SettingsPage {
    ...
    title: "Window Rules"
    readonly property var rules: kcm.revision >= 0 ? kcm.children("", "window-rule") : []

    function openRule(path) {
        kcm.push("pages/RuleEditorPage.qml", { rulePath: path });
    }
    ...
    CardHeader {
        ...
        trailing: QQC2.Button {
            ...
            text: "Add rule"
            onClicked: {
                const path = kcm.append("", Kdl.block("window-rule", [Kdl.leaf("match", [], {})]));
                if (path.length) {
                    page.openRule(path);
                }
            }
    ...
    component RuleCard: QQC2.ItemDelegate {
        ...
        contentItem: RowLayout {
            ...
            Item {
                ...
                Repeater {
                    ...
                    Kirigami.Icon {
                        ...
                        source: kcm.live.iconFor(modelData)
            ...
            ColumnLayout {
                ...
                QQC2.Label {
                    text: RuleSummary.target(card.rule, appId => kcm.live.nameFor(appId))
                    font.bold: true
