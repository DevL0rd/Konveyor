...
function write(kcm, scopePath, key, value) {
    return kcm.setNode(scopePath + "/" + key, nodeFor(key, value));
}

function writeFlag(kcm, scopePath, overrideMode, key, on) {
    if (on || overrideMode) {
        return kcm.setValue(scopePath + "/" + key, on ? [] : [false]);
    }
    return kcm.remove(scopePath + "/" + key);
}
