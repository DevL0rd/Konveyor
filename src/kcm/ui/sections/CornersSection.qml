...
ColumnLayout {
    ...
    readonly property var rule: kcm.revision >= 0 ? CornerRule.find(kcm) : ({ path: "", radii: [0, 0, 0, 0], clip: false })

    function writeRule(name, args) {
        if (root.rule.path.length > 0) {
            kcm.setValue(root.rule.path + "/" + name, args);
        } else {
            kcm.append("", Kdl.block("window-rule", [Kdl.leaf(name, args)]));
        }
    }
