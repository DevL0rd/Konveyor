...
QQC2.Button {
    ...
    readonly property var running: {
        const seen = new Set();
        const list = [];
        for (const window of SettingsStore.live.windows) {
            if (!window.app_id || seen.has(window.app_id)) {
                continue;
            }
            seen.add(window.app_id);
            list.push({ appId: window.app_id, name: SettingsStore.live.nameFor(window.app_id), detail: window.title, icon: SettingsStore.live.iconFor(window.app_id), group: "Open now" });
        }
        return list;
    }
    ...
    function entries(query) {
        ...
        const installed = SettingsStore.live.applications
    ...
    icon.name: appId.length ? SettingsStore.live.iconFor(appId) : "application-x-executable"
    text: appId.length ? SettingsStore.live.nameFor(appId) + (SettingsStore.live.nameFor(appId) === appId ? "" : "  (" + appId + ")") : placeholder
    onClicked: {
        SettingsStore.live.refresh();
        search.text = "";
        popup.open();
