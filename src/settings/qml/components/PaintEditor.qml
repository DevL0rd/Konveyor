...
RowLayout {
    ...
    function writeColor(text) {
        SettingsStore.remove(blockPath + "/" + gradientName);
        SettingsStore.setValue(blockPath + "/" + colorName, [text]);
    }
    ...
    Segmented {
        ...
        onChosen: value => {
            if (value === "none") {
                SettingsStore.remove(editor.blockPath + "/" + editor.colorName);
                SettingsStore.remove(editor.blockPath + "/" + editor.gradientName);
            } else if (value === "theme") {
