...
ColumnLayout {
    ...
    SwitchRow {
        ...
        onSwitched: on => on ? SettingsStore.setNode(editor.blockPath, Kdl.block(editor.blockPath.split("/").pop(), [])) : SettingsStore.remove(editor.blockPath)
    }
    ...
    SettingRow {
        ...
        TriState {
            ...
            onStateChosen: state => {
                SettingsStore.remove(editor.blockPath + "/on");
                SettingsStore.remove(editor.blockPath + "/off");
                if (state !== null) {
                    SettingsStore.append(editor.blockPath, Kdl.leaf(state ? "on" : "off"));
                }
            }
        }
    }
    ...
    SettingRow {
        ...
        RowLayout {
            Segmented {
                ...
                onChosen: value => value === "inherit" ? SettingsStore.remove(editor.blockPath + "/width") : SettingsStore.setValue(editor.blockPath + "/width", [widthSpin.value])
            }
            ...
            QQC2.SpinBox {
                ...
                onValueModified: SettingsStore.setValue(editor.blockPath + "/width", [value])
            }
        }
    }
