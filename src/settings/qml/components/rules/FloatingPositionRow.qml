...
SettingRow {
    ...
    function write(changes) {
        ...
        SettingsStore.setValue(path, [], { x: Math.round(next.x), y: Math.round(next.y), "relative-to": next["relative-to"] });
    }
    ...
    RowLayout {
        ...
        Segmented {
            ...
            onChosen: value => value === "default" ? SettingsStore.remove(row.path) : row.write({})
        }
