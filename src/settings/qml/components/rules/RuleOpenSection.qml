...
ColumnLayout {
    ...
    readonly property var workspaceNames: SettingsStore.revision >= 0 ? SettingsStore.children("", "workspace").map(entry => String((entry.node.args || [])[0] || "")).filter(name => name.length) : []

    function argOf(name) {
        const node = SettingsStore.revision >= 0 ? SettingsStore.node(rulePath + "/" + name) : ({});
    ...
    SettingRow {
        ...
        Segmented {
            ...
            onChosen: value => value === "default" ? SettingsStore.remove(section.rulePath + "/manage") : SettingsStore.setValue(section.rulePath + "/manage", [value === "tile"])
        }
    }
    ...
    SettingRow {
        ...
        Segmented {
            ...
            onChosen: value => value === "default" ? SettingsStore.remove(section.rulePath + "/default-column-display") : SettingsStore.setValue(section.rulePath + "/default-column-display", [value])
        }
    }
    ...
    SettingRow {
        ...
        Segmented {
            ...
            onChosen: value => value === "default" ? SettingsStore.remove(section.rulePath + "/group-app-windows") : SettingsStore.setValue(section.rulePath + "/group-app-windows", [value])
        }
    }
